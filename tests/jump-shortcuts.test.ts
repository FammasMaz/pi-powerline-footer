...
test("context usage changes repaint from live streaming message usage", () => {
  ...
  assert.match(source, /pi\.on\("agent_start", async \(_event, ctx\) => \{[\s\S]*?isStreaming = true;[\s\S]*?liveAssistantUsage = null;[\s\S]*?onVibeAgentStart\(\);[\s\S]*?dismissWelcome\(ctx\);[\s\S]*?currentCtx = ctx;/);
  assert.match(source, /pi\.on\("message_update", async \(event, ctx\) => \{[\s\S]*?liveAssistantUsage = event\.message\.usage;[\s\S]*?statusRenderScheduler\.schedule\(CONTEXT_STATUS_RENDER_MS\)/);
  assert.match(source, /pi\.on\("message_end", async \(event, ctx\) => \{[\s\S]*?streamMetrics\.onMessageEnd[\s\S]*?requestImmediateStatusRender\(\{ deferDuringTyping: false \}\)/);
  assert.match(source, /pi\.on\("agent_end", async \(_event, ctx\) => \{[\s\S]*?isStreaming = false;[\s\S]*?liveAssistantUsage = null;[\s\S]*?currentCtx = ctx;/);
